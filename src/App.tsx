...
// ─── ACTIVE SECTION ───────────────────────────────────────────
const SECTIONS = ['about','experience','education','projects','skills','publications']
const ALL_SECTIONS = [...SECTIONS, 'contact']
...
function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const ACCESS_KEY = '72a32976-190a-495d-8d49-0f326388044c'
  ...
  const inputStyle: React.CSSProperties = {
    width: '100%', fontFamily: 'var(--sans)', fontSize: '0.86rem',
  ...
  return (
    <section id="contact" style={{ padding: '0 40px 120px', maxWidth: 1300, margin: '0 auto' }}>
      ...
      <Reveal>
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 24, padding: 'clamp(32px,5vw,60px)', display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 52, boxShadow: '0 8px 48px var(--shadow)' }}>
          ...
          <div>
            ...
            <p style={{ fontFamily: 'var(--sans)', fontWeight: 300, fontSize: '0.9rem', color: 'var(--muted)', lineHeight: 1.9, marginBottom: 28 }}>
              Whether it's a role, a research collaboration, or just a question — send me a message and I'll get back to you within 24 hours.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {/* Email — opens Gmail compose in new tab like LinkedIn */}
              <a href="https://mail.google.com/mail/?view=cm&to=guduruharshita28@gmail.com&subject=Opportunity%20for%20Harshita%20Guduru"
                target="_blank" rel="noreferrer"
                style={{ fontFamily: 'var(--sans)', fontSize: '0.8rem', color: 'var(--muted)', textDecoration: 'none', display: 'flex', gap: 14, alignItems: 'center', transition: 'color 0.2s', padding: '12px 0', borderBottom: '1px solid var(--border)', cursor: 'pointer' }}
                onMouseEnter={e => (e.currentTarget.style.color = 'var(--accent)')}
                onMouseLeave={e => (e.currentTarget.style.color = 'var(--muted)')}>
                <span style={{ fontSize: '0.57rem', color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: '0.12em', width: 54, flexShrink: 0 }}>Email</span>
              ...
              <a href={PROFILE.contact.linkedin} target="_blank" rel="noreferrer"
                ...
                onMouseEnter={e => (e.currentTarget.style.color = 'var(--accent)')}
                onMouseLeave={e => (e.currentTarget.style.color = 'var(--muted)')}>
                <span style={{ fontSize: '0.57rem', color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: '0.12em', width: 54, flexShrink: 0 }}>LinkedIn</span>
              ...
              <a href={PROFILE.contact.github} target="_blank" rel="noreferrer"
                ...
                onMouseEnter={e => (e.currentTarget.style.color = 'var(--accent)')}
                onMouseLeave={e => (e.currentTarget.style.color = 'var(--muted)')}>
                <span style={{ fontSize: '0.57rem', color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: '0.12em', width: 54, flexShrink: 0 }}>GitHub</span>
                {PROFILE.contact.githubLabel}
              </a>
              {/* Phone */}
              <a href="tel:+19472405093"
                style={{ fontFamily: 'var(--sans)', fontSize: '0.8rem', color: 'var(--muted)', textDecoration: 'none', display: 'flex', gap: 14, alignItems: 'center', transition: 'color 0.2s', padding: '12px 0', borderBottom: '1px solid var(--border)', cursor: 'pointer' }}
                onMouseEnter={e => (e.currentTarget.style.color = 'var(--accent)')}
                onMouseLeave={e => (e.currentTarget.style.color = 'var(--muted)')}>
                <span style={{ fontSize: '0.57rem', color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: '0.12em', width: 54, flexShrink: 0 }}>Phone</span>
                {PROFILE.contact.phone}
              </a>
              {/* Location — no link */}
              <div style={{ fontFamily: 'var(--sans)', fontSize: '0.8rem', color: 'var(--muted)', display: 'flex', gap: 14, alignItems: 'center', padding: '12px 0', borderBottom: '1px solid var(--border)' }}>
                <span style={{ fontSize: '0.57rem', color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: '0.12em', width: 54, flexShrink: 0 }}>Location</span>
                {PROFILE.location}
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ fontFamily: 'var(--sans)', fontSize: '0.62rem', fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Name *</label>
                <input name="name" value={form.name} onChange={handleChange} placeholder="Your name" required
                  style={inputStyle}
                  onFocus={e => { e.currentTarget.style.borderColor = 'var(--accent-bd)'; e.currentTarget.style.boxShadow = '0 0 0 3px var(--accent-bg)' }}
                  onBlur={e => { e.currentTarget.style.borderColor = 'var(--border2)'; e.currentTarget.style.boxShadow = 'none' }}
                />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ fontFamily: 'var(--sans)', fontSize: '0.62rem', fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Email *</label>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontFamily: 'var(--sans)', fontSize: '0.62rem', fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Subject</label>
              <input name="subject" value={form.subject} onChange={handleChange} placeholder="Job opportunity, collaboration, etc."
                style={inputStyle}
                onFocus={e => { e.currentTarget.style.borderColor = 'var(--accent-bd)'; e.currentTarget.style.boxShadow = '0 0 0 3px var(--accent-bg)' }}
                onBlur={e => { e.currentTarget.style.borderColor = 'var(--border2)'; e.currentTarget.style.boxShadow = 'none' }}
              />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontFamily: 'var(--sans)', fontSize: '0.62rem', fontWeight: 700, color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.12em' }}>Message *</label>
              <textarea name="message" value={form.message} onChange={handleChange} placeholder="Tell me about the opportunity or what you'd like to discuss..." required rows={5}
            ...
            <button type="submit" disabled={status === 'sending'}
              style={{ fontFamily: 'var(--sans)', fontWeight: 700, fontSize: '0.9rem', color: 'white', background: status === 'sending' ? 'var(--dim)' : 'var(--accent)', padding: '14px 28px', borderRadius: 11, border: 'none', cursor: status === 'sending' ? 'not-allowed' : 'pointer', transition: 'all 0.2s', boxShadow: '0 4px 20px var(--accent-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
              onMouseEnter={e => { if (status !== 'sending') (e.currentTarget as HTMLElement).style.opacity = '0.85' }}
              onMouseLeave={e => (e.currentTarget as HTMLElement).style.opacity = '1'}>
              {status === 'sending' ? (
                <><span style={{ display: 'inline-block', width: 14, height: 14, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', animation: 'spinCW 0.7s linear infinite' }} /> Sending...</>
              ) : 'Send Message →'}
            </button>
          </form>
        </div>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => { setShowModal(false); setStatus('idle') }}>
          <div className="modal-box" style={{ maxWidth: 420, textAlign: 'center', padding: '48px 40px' }} onClick={e => e.stopPropagation()}>
            {status === 'success' ? (
              <>
                <div style={{ fontSize: '3.2rem', marginBottom: 16 }}>✉️</div>
                <h3 style={{ fontFamily: 'var(--serif)', fontWeight: 700, fontSize: '1.6rem', color: 'var(--ink)', letterSpacing: '-0.03em', marginBottom: 12 }}>Message Sent!</h3>
                <p style={{ fontFamily: 'var(--sans)', fontSize: '0.88rem', color: 'var(--muted)', lineHeight: 1.8, marginBottom: 28 }}>
                  Thanks for reaching out! I'll get back to you at <strong style={{ color: 'var(--accent)' }}>{form.email || 'your email'}</strong> within 24 hours.
                </p>
                <button onClick={() => { setShowModal(false); setStatus('idle') }}
                  style={{ fontFamily: 'var(--sans)', fontWeight: 700, fontSize: '0.86rem', color: 'white', background: 'var(--accent)', padding: '12px 28px', borderRadius: 10, border: 'none', cursor: 'pointer', transition: 'opacity 0.2s' }}
                  onMouseEnter={e => (e.currentTarget.style.opacity = '0.8')}
                  onMouseLeave={e => (e.currentTarget.style.opacity = '1')}>
                  Close
                </button>
              ...
              <>
                <div style={{ fontSize: '3.2rem', marginBottom: 16 }}>⚠️</div>
                <h3 style={{ fontFamily: 'var(--serif)', fontWeight: 700, fontSize: '1.6rem', color: 'var(--ink)', letterSpacing: '-0.03em', marginBottom: 12 }}>Couldn't Send</h3>
                <p style={{ fontFamily: 'var(--sans)', fontSize: '0.88rem', color: 'var(--muted)', lineHeight: 1.8, marginBottom: 8 }}>
                  Something went wrong. Please try again, or reach out directly:
                </p>
                <a href={`mailto:${PROFILE.contact.email}`} style={{ fontFamily: 'var(--sans)', fontWeight: 600, fontSize: '0.88rem', color: 'var(--accent)', display: 'block', marginBottom: 24 }}>{PROFILE.contact.email}</a>
                <button onClick={() => { setShowModal(false); setStatus('idle') }}
                  style={{ fontFamily: 'var(--sans)', fontWeight: 700, fontSize: '0.86rem', color: 'var(--ink2)', background: 'var(--surf2)', padding: '12px 28px', borderRadius: 10, border: '1px solid var(--border2)', cursor: 'pointer' }}>
                  Close
                </button>
              </>
            )}
      ...
      <footer style={{ borderTop: '1px solid var(--border)', padding: '24px 40px', background: 'var(--surf2)' }}>
        <div style={{ maxWidth: 1300, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 24 }}>
            {[['GitHub', PROFILE.contact.github],['LinkedIn', PROFILE.contact.linkedin],['Email', `mailto:${PROFILE.contact.email}?subject=Opportunity%20for%20Harshita%20Guduru`]].map(([l,h]) => (
              <a key={l} href={h}
